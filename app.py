# ============================================================
# 🎬 SISTEMA DE RECOMENDACIÓN VISUAL — STREAMLIT APP (CSV + Cluster + Filtro Género)
# ============================================================
...
def get_poster_path(imdb_id, split="train"):
    """Devuelve la ruta del póster si existe."""
    base = POSTERS_TRAIN if split == "train" else POSTERS_TEST
    path = base / f"{imdb_id}.jpg"
...
def load_all_data():
    ...
    # Detectar columnas de embeddings (UMAP o PCA)
...
# ============================================================
st.set_page_config(page_title="🎥 Recomendador (Test → Train con Cluster + Género)", layout="wide")
...
# Filtro de género (basado en texto tipo Drama|Comedy|Action)
...
if modo == "Visualizador (TEST)":
    ...
    for i, (_, row) in enumerate(muestras.iterrows()):
        imdb_id = row["imdbId"]
        poster_path = get_poster_path(imdb_id, "test")
        with cols[i % 5]:
            if poster_path:
...
# ============================================================
    ...
    st.title("🎯 Recomendaciones — corpus TRAIN (Cluster + Género textual)")
    ...
    poster_base = get_poster_path(base_id, "test")

    # --- Panel lateral ---
    st.sidebar.image(str(poster_base) if poster_base else np.zeros((200,150,3)), use_container_width=True)
    ...
    # ============================================================
    # 🔹 1. Determinar cluster base (más cercano en el train)
    # ============================================================
    ...
    # ============================================================
    # 🔹 3. Calcular distancias dentro del cluster
    # ============================================================
    X_cluster = df_cluster[emb_cols].values
    imdb_cluster = df_cluster["imdbId"].values
    ...
    # ============================================================
    # 🔹 4. Aplicar filtro de género textual (dentro del cluster)
    # ============================================================
    if filtro_genero != "Todos":
        df_cluster = df_cluster[
            df_cluster["Genre"]
            .fillna("")
            .apply(lambda g: filtro_genero.lower() in [x.strip().lower() for x in g.split("|")])
        ]
    ...
    for i, (_, row) in enumerate(df_cluster.iterrows()):
        imdb_id_rec = row["imdbId"]
        poster_path = get_poster_path(imdb_id_rec, "train")
        with cols[i % 4]:
            if poster_path:
                st.image(str(poster_path), use_container_width=True)
            else:
